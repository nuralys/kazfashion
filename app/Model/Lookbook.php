...
class Lookbook extends AppModel{
	...
	public function customUploadImg($file = array()){
		...
		$path = WWW_ROOT . 'img/lookbook/' . $fileName;
		$path_th = WWW_ROOT . 'img/lookbook/thumbs/' . $fileName;
		if(!move_uploaded_file($file['img']['tmp_name'], $path)){
			return false;
		}
		$this->resizeImg($path, $path_th, 118, 177, $ext);
	...
	public function genNameFile($ext){
		...
		if(is_file(WWW_ROOT . 'img/lookbook/' . $name)){
